...
import (
	"SimilarImageFilter/ImageFilter/Library/goimagehash/etcs"
	"SimilarImageFilter/ImageFilter/Library/goimagehash/transforms"
	"errors"
...
func AverageHash(img image.Image) (*ImageHash, error) {
	...
	//resized := resize.Resize(8, 8, img, resize.NearestNeighbor)
	resized := imaging.Resize(img, 8, 8, imaging.NearestNeighbor)
...
func PerceptionHash(img image.Image) (*ImageHash, error) {
	...
	resized := imaging.Resize(img, 64, 64, imaging.NearestNeighbor)
	pixels := transforms.Rgb2Gray(*resized)
...
func ExtPerceptionHash(img image.Image, width, height int) (*ExtImageHash, error) {
	...
	var phash []uint64
	//resized := resize.Resize(uint(imgSize), uint(imgSize), img, resize.NearestNeighbor)
	resized := imaging.Resize(img, width, height, imaging.NearestNeighbor)
	pixels := transforms.Rgb2Gray(*resized)
	// dct := transforms.DCT2D(pixels, imgSize, imgSize)
	dct := transforms.DCT2D(pixels, width, height)
...
func ExtDifferenceHash(img image.Image, width, height int) (*ExtImageHash, error) {
	...
	//resized := resize.Resize(uint(width)+1, uint(height), img, resize.NearestNeighbor)
	resized := imaging.Resize(img, width+1, height, imaging.NearestNeighbor)
	pixels := transforms.Rgb2Gray(*resized)
